...
from Scalar_Trajectories import  KerrCircEqFluxScalar, KerrCircEqFluxMassiveScalar18, KerrCircEqFluxMassiveScalar36 
import h5py
...
def parse_arguments():
    ...
    #scalar charge
    parser.add_argument('--use_scalar_charge', action='store_true', default=False, help="Consider beyond-vacuum GR scalar charge emission")
    parser.add_argument("--Lambda", help="scalar_charge_squared", type=float, default=0.0)
    parser.add_argument("--ScalarMass", help="scalar_charge_mass", type=float, default=0.0)
...
if __name__ == "__main__":
    ...
    if args.power_law and args.use_scalar_charge:
        raise ValueError("Cannot use both --power_law and --use_scalar_charge at the same time.")
    ...
    FluxClass = KerrEccEqFlux
    additional_args = ()

    A = 0
    nr = args.nr
    Lambda = args.Lambda
    ScalarMass = args.ScalarMass
    #Ndelta = args.Ndelta

    if args.power_law: 
        A = 0.0 
        FluxClass = KerrEccEqFluxPowerLaw
        additional_args = (A, nr) # A is the power-law coefficient, nr is the power-law exponent

    if args.use_scalar_charge: 
        if args.ScalarMass == 0.018: 
            FluxClass = KerrCircEqFluxMassiveScalar18 #KerrCircEqFluxScalarCharge
        elif args.ScalarMass == 0.036: 
            FluxClass = KerrCircEqFluxMassiveScalar36 
        elif args.ScalarMass == 0.0: #and args.e_f == 0.0: 
            FluxClass =  KerrCircEqFluxScalar
        #elif args.ScalarMass == 0.0: 
        #    FluxClass = KerrEccEqFluxScalar     
        else: 
            raise ValueError(f"Unsupported ScalarMass = {ScalarMass}. Supported values are: 0.018, 0.036, 0.0")       
        
        additional_args = (Lambda, ScalarMass)

    FluxClass().additional_args = additional_args  

    inspiral_kwargs_back = {"err": 1e-13,"integrate_backwards": True, "func":  FluxClass}
    inspiral_kwargs_forward = {"err": 1e-13,"integrate_backwards": False, "func":  FluxClass}     

    param_names = np.array(['M','mu','a','p0','e0','xI0','dist','qS','phiS','qK','phiK','Phi_phi0','Phi_theta0','Phi_r0', 'A', 'nr','Lambda', 'ScalarMass'])
    ...
    elif args.use_scalar_charge:
        #popinds.append(2) #schwarschild only
        if args.ScalarMass == 0.0 and args.e_f != 0.0: #eccentric massless, not implemented yet
            popinds.append(14) #remove A
            popinds.append(15) #remove nr
            popinds.append(17) #remove ScalarMass 
        elif args.ScalarMass == 0.0 and args.e_f == 0.0: #circular massless 
            popinds.append(4) #remove e0  
            popinds.append(13) #remove Phi_r0
            popinds.append(14) #remove A 
            popinds.append(15) #remove nr
            popinds.append(17) #remove ScalarMass
        else:  #massive circular
            popinds.append(4) #remove e0  
            popinds.append(13) #remove Phi_r0 
            popinds.append(14) #remove A
            popinds.append(15) #remove nr    
    ...
    if args.power_law: #Beyond-GR only considered for circular case
        e_f = 0.0
    elif args.use_scalar_charge: #scalar charge case (both massless and massive ) is circular only 
        e_f = 0.0  
    else:
        e_f = args.e_f 
    x0_f = 1.0 * np.sign(args.a) if args.a != 0.0 else 1.0
    # initialize the trajectory
    traj = EMRIInspiral(func=FluxClass)
    ...
    print("nr=", nr)
    print("Lambda=", Lambda)
    print("ScalarMass=", ScalarMass)
    t_forward, p_forward, e_forward, x_forward, Phi_phi_forward, Phi_r_forward, Phi_theta_forward = traj(M, mu, a, p_f, e_f, x0_f, A, nr, Lambda, ScalarMass,dt=1e-5, T=100.0, integrate_backwards=False)
    t_back, p_back, e_back, x_back, Phi_phi_back, Phi_r_back, Phi_theta_back = traj(M, mu, a, p_forward[-1], e_forward[-1], x_forward[-1], A, nr, Lambda,ScalarMass, dt=1e-5, T=Tpl, integrate_backwards=True)
    # and forward trajectory to check the evolution
    t_plot, p_plot, e_plot, x_plot, Phi_phi_plot, Phi_r_plot, Phi_theta_plot = traj(M, mu, a, p_back[-1], e_back[-1], x_back[-1], A, nr, Lambda, ScalarMass, dt=1e-5, T=T, integrate_backwards=False)
    ...
    print(phiK,phiS)
    parameters = np.asarray([M, mu, a, p0, e0, x0, dist, qS, phiS, qK, phiK, Phi_phi0, Phi_theta0, Phi_r0, A, nr, Lambda,ScalarMass])
    # evaluate waveform
    ...
    print("max_f =",max_f )
    print("fnyquist =",1/(args.dt*2)) 
    print("f nyquist must be larger than max_f. aliasing otherwise")
    print("f_max =",fmax)
    ...
    # test with and without charge 
    if args.use_scalar_charge and ScalarMass == 0.0:
        test_parameters = np.asarray([M, mu, a, p0, e0, x0, dist, qS, phiS, qK, phiK, Phi_phi0, Phi_theta0, Phi_r0, 0.0, 8.0,Lambda])
        test_waveform_1 = model(*test_parameters)
        test_parameters = np.asarray([M, mu, a, p0, e0, x0, dist, qS, phiS, qK, phiK, Phi_phi0, Phi_theta0, Phi_r0, 0.0, 8.0,0.0])
        ...
        print("Overlap between GR and beyond-GR waveform with Lambda:", olap.get()) 
    ...
    for j in range(args.N_montecarlo):
        ...
        #fixed angles 
        Phi_phi0, Phi_r0, Phi_theta0 = [2.5,0.0,0.0]
        qS, phiS, qK, phiK = [0.8,2.2,1.6,1.2]
        ...
        fish = StableEMRIFisher(*parameters[:-4], add_param_args = {"A": A, "nr": nr, "Lambda":Lambda, "ScalarMass":ScalarMass}, fmin=fmin, fmax=fmax,
                                ...
                                der_order=der_order, Ndelta=35, filename=current_folder,
                                deltas = deltas,
        ...
        if args.calculate_fisher:
            ...
            if args.power_law: 
                ...
                if args.ScalarMass == 0.0 and args.e_f == 0.0:  #circular massless
                    J = cosmo.jacobian_powerlaw(M / (1 + args.z), mu / (1 + args.z), args.z) #jacobian_schwarzschild per schwarzschild 
                elif args.ScalarMass == 0.0 and args.e_f != 0.0: #eccentric massless
                    J = cosmo.jacobian_kerr_eccentric_scalar(M / (1 + args.z), mu / (1 + args.z), args.z)
                else: #massive circular
            ...
            errors_df.to_markdown(os.path.join(current_folder, f"summary_M{M:.0e}_mu{mu}_a{a}_ef{e_f}_T{T}_Lambda{Lambda}_ScalarMass{ScalarMass}_z{args.z}_der_order{der_order}.md"), floatfmt=".10e")
            ...
            np.savez(os.path.join(current_folder, f"results_M{M:.0e}_mu{mu}_a{a}_ef{e_f}_T{T}_Lambda{Lambda}_ScalarMass{ScalarMass}_z{args.z}_der_order{der_order}.npz"), gamma=fim, cov=cov, snr=SNR, fisher_params=fisher_params, errors=errors, relative_errors=relative_errors, names=param_names, source_frame_cov=source_frame_cov, err_sky_loc=err_sky_loc, redshift=args.z, e_f=args.e_f)
            
            # save to h5 file
            with h5py.File(os.path.join(current_folder, f"results_M{M:.0e}_mu{mu}_a{a}_ef{e_f}_T{T}_Lambda{Lambda}_ScalarMass{ScalarMass}_z{args.z}_der_order{der_order}.h5"), "w") as f:
